...
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.APP_ID;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.DATASET_ID;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.DATASET_NAME;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.INODE_NAME;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.LOGICAL_TIME;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.LOGICAL_TIME_BATCH;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.OPERATION;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.P1_NAME;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.P2_NAME;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.PARTITION_ID;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.PROJECT_ID;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.TIMESTAMP;
import static io.hops.metadata.hdfs.TablesDef.FileProvenanceTableDef.TIMESTAMP_BATCH;
...
public class FileProvenanceClusterj implements TablesDef.FileProvenanceTableDef,
  ...
  private Collection<FileProvenanceEntry> createCollection(Collection<ProvenanceLogEntryDto> collection) {
    ArrayList<FileProvenanceEntry> list = new ArrayList<>(collection.size());
  ...
  private FileProvenanceEntry createProvenanceLogEntry(ProvenanceLogEntryDto dto) {
    return new FileProvenanceEntry(
      dto.getInodeId(),
      FileProvenanceEntry.Operation.valueOf(dto.getOperation()),
      dto.getLogicalTime(),
      dto.getTimestamp(),
      dto.getAppId(),
      dto.getUserId(),
