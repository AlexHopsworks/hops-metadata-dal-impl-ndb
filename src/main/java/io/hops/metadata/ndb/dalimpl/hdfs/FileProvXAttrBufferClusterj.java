...
public class FileProvXAttrBufferClusterj implements TablesDef.FileProvXAttrBufferTableDef,
  ...
  private Collection<FileProvXAttrBufferEntry> createCollection(Collection<FileProvXAttrBufferEntryDto> collection) {
    ArrayList<FileProvXAttrBufferEntry> list = new ArrayList<>(collection.size());
    for (FileProvXAttrBufferEntryDto dto : collection) {
      list.add(createProvenanceLogEntry(dto));
    }
    return list;
  }

  private FileProvXAttrBufferEntry createProvenanceLogEntry(FileProvXAttrBufferEntryDto dto) {
    return new FileProvXAttrBufferEntry(
      dto.getINodeId(),
      dto.getNamespace(),
      dto.getName(),
      dto.getINodeLogicalTime(),
      dto.getValue());
  }
